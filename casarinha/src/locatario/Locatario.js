...
function Locatario() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:3000/locatario/")
  ...
  async function handleSubmit(event) {
    ...
    const response = await fetch("http://localhost:3000/locatario/", {method: "POST",
  ...
  async function deleteChange(event){
    event.preventDefault();
    const response = await fetch("http://localhost:3000/locatario/", {method: "DELETE",
    body: JSON.stringify({CPFLocatario: event.target.id })});
    const json = await response.text();
    setError(json);
    if(error !== 'Confirmado'){
      alert(json)
  ...
  return (
    <main className="main">
      <h1 className='title'>Locatários Cadastrados:</h1>
      <table>
        <tbody>
          <tr >            
